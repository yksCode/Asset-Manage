<template>
  ...
  <view class="container"> 
    ...
    <view v-if="data.list.length == 0" style="padding: 40% 0 80% 0;">
	   ...
	   <view class="scrollBox img-center" style="bottom: 8%;right:12%;" @tap="navTo('../type/type_add')">
      ...
      <view class="item" v-for="(item, index) in data.list" :key="item._id">
		<view class="content-list">
          <view class="information">
            ...
            <view class="st-title">
              <view class="title">【{{ index + 1 }}】设备类别-{{ item.typename }}</view>
	...
	export default {
		data() {
			...
			return {
				...
				typeid:"",
				typestatus:"",
				url: "client/type/sys/getList", // 获取list数据的云函数请求路径
				// init请求返回的数据
				data: {
					list: [], // 列表数据
