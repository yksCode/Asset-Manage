       ...
       <view class="content-list" @tap="goDetail(item)">
          <view class="information">          
            <view class="st-title">
              ...
              <view class="time">领用时间：{{ item._add_time_str }}</view>
              <view class="time" v-if=" item.maintenanceTime==undefined " v-show="false">最近维护时间：{{ item.maintenanceTime }}</view>
			  <view class="time" v-else v-show="true">最近维护时间：{{ item.maintenanceTime }}</view>
              <view class="description"><text space="ensp">责任人：</text>{{ item.principal }}</view>
			  ...
			  <view class="description" v-else>
			  	  <text space="ensp" style="color: #fa3534;">设备状态：{{ item.status }}</text>
			  </view>
            </view>
          </view>
	...
	export default {
		data() {
			...
			return {
				...
				typeid:"",
				typestatus:"",
				url: "client/device/sys/getList", // 获取list数据的云函数请求路径
				// init请求返回的数据
				data: {
					list: [], // 列表数据
