  ...
  <view class="container">
    <view class="grid" style="padding-left: 30rpx;padding-right: 30rpx;">
		<u-form :model="form" ref="uForm">
			...
			<u-form-item label="责任人:" prop="principal" label-width="130">				
					<u-input v-model="form.principal" placeholder="请填写责任人" input-align="right" />	
			</u-form-item>			
		  ...
		  vk.callFunction({
		    ...
		    success(data) {
		       if(!data.item){
				   ...
				   that.form = data.item;
				   that.result = data.item.typename;
				   that.organResult = data.item.organ;
			   }		       	      
		    }
		  });
		},  		
	      	...
	      	if (valid) {
	      		console.log('验证通过');
	      				...
	      				var up_data = that.form;
						    up_data.typename = that.result;
						    up_data.organ = that.organResult;
						    up_data._id = that._id;
	      				vk.callFunction({
	      				  url: 'client/device/sys/updateById',
	      				  data: up_data,
	      				  title: '请求中...',
	      				  success(data) {
	      				    uni.showToast({
