       ...
       <view class="content-list">
          <view class="information">          
            <view class="st-title">
              ...
              <view class="time">申请时间：{{ item._add_time_str }}</view>
			  <view class="time">报修人：{{ item.principal }}</view>
			  <view class="time">存放地点：{{ item.address }}</view>
			  <view class="description" v-if=" item.status=='已处理' ">
				  <text space="ensp" style="color: #19be6b;">处理状态：{{ item.status }}</text>
			  </view>
			  <view class="description" v-else>
			  	  <text space="ensp" style="color: #fa3534;">状态：{{ item.status }}</text>
			  </view>
			  <view class="time" v-if=" item.maintenanceTime==undefined " v-show="false">处理时间：{{ item.maintenanceTime }}</view>
			  <view class="time" v-else v-show="true">处理时间：{{ item.maintenanceTime }}</view>
            </view>
          </view>
	...
	export default {
		data() {
			...
			return {
				...
				typestatus:"",
				dev_id:'',
				url: "client/device/sys/getApplyList", // 获取list数据的云函数请求路径
				// init请求返回的数据
				data: {
					list: [], // 列表数据
		...
		methods: {
			...
			updateById() {
			  ...
			  vk.callFunction({
			    url: 'client/device/sys/updateApplyById',
			    title: '请求中...',
			    data: data,
			    success(data) {
