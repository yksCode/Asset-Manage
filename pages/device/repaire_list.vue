	...
	export default {
		data() {
			...
			return {
				...
				typestatus:"",
				dev_id:'',
				url: "client/device/sys/getRepaireList", // 获取list数据的云函数请求路径
				// init请求返回的数据
				data: {
					list: [], // 列表数据
		...
		methods: {
			...
			updateStatusById() {
			  ...
			  vk.callFunction({
			    ...
			    success(data) {
				   that.sendMessage();
				   that.updateSubscribeStatusById()
			       console.log(data)
			    }
			  });
			},
			updateSubscribeStatusById() {
			  let data = {
			    dev_id: this.dev_id				
			  };			 
			  vk.callFunction({
			    url: 'client/device/sys/updateSubscribeStatusById',
			    title: '请求中...',
			    data: data,
			    success(data) {				   
			       console.log(data)
			...
			findSubscribeById(){
				...
				vk.callFunction({
				  url: 'client/device/sys/FindSubscribeById',
				  title: '请求中...',
				  data: data,
				  success(data) {
					 that.item = data.item;
			...
			sendMessage(){
				...
				that.item.phrase16.DATA = '已完成'
				vk.callFunction({
					url: "client/device/pub/sendMessage",
					title: "请求中...",
					data: that.item,
